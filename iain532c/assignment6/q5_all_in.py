...
class Course:
    def __init__(self, name, faculty, hours, constraining_value=inf):
        self.name = name
        self.faculty = faculty
        self.hours = hours
        self.constraining_value = constraining_value
    ...
    def clone(self):
        return Course(self.name, self.faculty, self.hours, self.constraining_value)

    def __lt__(self, other):
        if self.constraining_value != other.constraining_value:
            return self.constraining_value < other.constraining_value
        else:
...
class Timetable:
    ...
    def get_selected_slots(self):
        ...
        slot_list = [self.slots[batch_index][day][time].clone() for time in range(7) for day in range(5) for batch_index
                     in range(len(self.batches))]
        ...
        for slot in new_list:
            ...
            for b_day in range(5):
                for b_time in range(7):
                    ...
                    if (slot.day, slot.time) == (b_day, b_time):
                        continue

                    if batch.schedule[b_day][b_time] is not None:
                        continue

                    a_courses = slot.batch.get_courses(slot.day, slot.time)
                    b_courses = batch.get_courses(b_day, b_time)
        ...
        for slot in new_list:
            ...
            for b_batch_index in range(self.number_of_batches):
                ...
                if slot.batch.name == self.batches[b_batch_index].name:
                    continue

                b_slot = self.slots[b_batch_index][slot.day][slot.time]

                if b_slot.is_allotted:
                    continue

                a_courses = slot.batch.get_courses(slot.day, slot.time)
                b_courses = b_slot.batch.get_courses(slot.day, slot.time)
        ...
        for course in available_courses:
            ...
            for b_day in range(5):
                for b_time in range(7):
                    ...
                    if slot.batch.schedule[b_day][b_time] is not None:
                        continue

                    b_courses = slot.batch.get_courses(b_day, b_time)

                    for c in b_courses:
                        if c.name == course.name:
                            course.constraining_value += 1

        for course in available_courses:
            for b_batch in self.batches:

                if slot.batch == b_batch:
                    continue
                if b_batch.schedule[slot.day][slot.time] is not None:
                    continue

                b_courses = b_batch.get_courses(slot.day, slot.time)
                for c in b_courses:
                    if course.faculty.name == c.faculty.name:
                        course.constraining_value += 1


        # for course in available_courses:
        #     print(course.name, ':', course.constraining_value, ',', end=' ')
        # print()
