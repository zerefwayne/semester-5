...
def minimax(board, player_turn, depth):
    ...
    if is_maximizing_player(player_turn):
        ...
        for child in children:
            (val, x, y) = minimax(child[0], 'O', depth-1)
        ...
        for child in children:
            (val, x, y) = minimax(child[0], 'X', depth-1)
            if val < min_val:
                min_val = val
                next_x = child[1]
                next_y = child[2]
            ...
            if result is not None:
                ...
                if self.turn == 'X':
                    ...
                    while True:
                        ...
                        start = time.time()
                        (_, suggested_x, suggested_y) = minimax(self.board, self.turn, self.depth)
                        end = time.time()
